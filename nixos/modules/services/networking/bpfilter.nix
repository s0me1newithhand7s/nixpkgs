...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = {
      "bpfilter" = {
        description = "bpfilter eBPF Kernel Firewall Translation Daemon";
        ...
        serviceConfig =
          let
            rulesScript = pkgs.writeTextFile {
              ...
              text = ''
                ${compiledChainsText}
                ${lib.optionalString (cfg.ruleset != null) cfg.ruleset}
              '';
            };
          in
          {
            ExecStart = "${lib.getExe' pkgs.bpfilter "bpfilter"} --no-iptables --no-nftables";

            ExecStartPost =
              [ ]
              ++ lib.optionals (cfg.chains != { }) (
                lib.optional cfg.chainFlush "${lib.getExe' pkgs.bpfilter "bpcli"} chain flush"
                ++ lib.optional cfg.chainCheck "${lib.getExe' pkgs.bpfilter "bpcli"} chain set --dry-run --from-file ${rulesScript}"
                ++ [ "${lib.getExe' pkgs.bpfilter "bpcli"} chain set --from-file ${rulesScript}" ]
              )
              ++ lib.optionals (cfg.ruleset != null) (
                lib.optional cfg.rulesetFlush "${lib.getExe' pkgs.bpfilter "bpcli"} ruleset flush"
                ++ lib.optional cfg.rulesetCheck "${lib.getExe' pkgs.bpfilter "bpcli"} ruleset set --dry-run --from-file ${rulesScript}"
                ++ [ "${lib.getExe' pkgs.bpfilter "bpcli"} ruleset set --from-file ${rulesScript}" ]
              );
      ...
      "bpfilter-nft-import" = lib.mkIf (cfg.translations.useNftables != null) {
        ...
        serviceConfig = {
          ...
          ExecStart = [
            "${lib.getExe' pkgs.nftables "nft"} --check -f ${cfg.nftablesFile}"
            "${lib.getExe' pkgs.nftables "nft"} -f ${cfg.nftablesFile}"
          ];
        };
      };
      ...
      "bpfilter-iptables-import" = lib.mkIf (cfg.translations.useIptables != null) {
        ...
        serviceConfig = {
          ...
          RemainAfterExit = true;
          ExecStart = [
            "${lib.getExe' pkgs.iptables "iptables-restore"} --test ${cfg.iptablesFile}"
            "${lib.getExe' pkgs.iptables "iptables-restore"} ${cfg.iptablesFile}"
          ];
        };
      };
    };
